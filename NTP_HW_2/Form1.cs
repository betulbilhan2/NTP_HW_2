...
namespace NTP_HW_2
{
    public partial class Form1 : Form
    {
        ...
        //    istemci.Credentials = new System.Net.NetworkCredential("betulbilhan@outlook.com", "asdgtQWERT");
        //    istemci.Port = 587;
        //    istemci.Host = "smtp.live.com";
        //    istemci.EnableSsl = true;

        //    msj.Body = log.ToString();
        //    msj.Subject = "Ntp Odev outputu";
        //    msj.From = new MailAddress("betulbilhan@outlook.com");
        //    msj.To.Add("btlbilhan23@gmail.com");
        ...
        // Helper: JSON için string kaçış işlemi (basit, güvenilir)
        ...
        // Brevo'ya e-posta gönderme (manuel JSON)
        private async Task<bool> SendEmailViaBrevoManualJsonAsync(string fromEmail, string fromName, string toEmail, string toName, string subject, string htmlContent, string textContent = "")
        {
            // ÖNERİ: environment variable kullan
            string apiKey = Environment.GetEnvironmentVariable("BREVO_API_KEY");
            // Eğer environment yoksa test için şu satırı kullanabilirsin (geçici):
            // string apiKey = "BURAYA_TEST_API_KEY";  // <-- üretim/depo'ya asla koyma!

            if (string.IsNullOrEmpty(apiKey))
            {
                MessageBox.Show("Brevo API anahtarı bulunamadı. Lütfen BREVO_API_KEY environment variable'ını ayarla veya kodda geçici anahtarı ekle.");
                return false;
            }

            // Manuel JSON inşa et (EscapeForJson ile güvenli hale getir)
            string json = "{"
                + "\"sender\": {\"email\": \"" + EscapeForJson(fromEmail) + "\", \"name\": \"" + EscapeForJson(fromName) + "\"},"
                + "\"to\": [{\"email\": \"" + EscapeForJson(toEmail) + "\", \"name\": \"" + EscapeForJson(toName) + "\"}],"
                + "\"subject\": \"" + EscapeForJson(subject) + "\","
                + "\"htmlContent\": \"" + EscapeForJson(htmlContent) + "\","
                + "\"textContent\": \"" + EscapeForJson(textContent) + "\""
                + "}";
            ...
            using (var client = new HttpClient())
            {
                ...
                try
                {
                    ...
                    {
                        MessageBox.Show("E-posta gönderilemedi. Status: " + response.StatusCode + "\n\n" + respBody);
                        return false;
                    }
                }
                catch (Exception ex)
                {
                    MessageBox.Show("E-posta gönderilirken hata: " + ex.Message);
                    return false;
                }
            }
        }
        ...
        {
            string fromEmail = "betul.bilhan@stu.fsm.edu.tr"; 
            string fromName = "Betül";
            string toEmail = "btlbilhan23@gmail.com";
            string toName = "Alıcı";

            string subject = "NTP Ödev outputu";
            string htmlContent = "<p>Program logu:</p><pre>" + EscapeForJson(log) + "</pre>";
            string textContent = "Program logu: " + log;

            bool ok = await SendEmailViaBrevoManualJsonAsync(fromEmail, fromName, toEmail, toName, subject, htmlContent, textContent);
